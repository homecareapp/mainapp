        ...
        async.waterfall([
            ...
            function(nextfunc) {
                ...
                user.comparePassword(password, function(err, isMatch) {
                    ...
                    nextfunc(new Error('Invalid username or password.'));
                });
            }
        ], function(err) {
...
exports.isAuthenticatedApi = function(req, res, next) {
    ...
    Provider.findById(key, function (err, result) {
        if (err) return res.status(401).json({
            error: 'Not authorized'
        });
        req.user={};
        req.user.provider_id = result._id;
        return next();
    })
};
/**
...
exports.isAuthorized = function(req, res, next) {
    ...
    // RolesMaster.find({},{privileges:1, name:1}).lean().populate([]).exec(function(err, rolesmaster) {
